...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin, onOpenSubscription }) => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'apple') => {
    ...
    try {
        await dataService.signInWithProvider(provider);
        // User will be redirected, no need to handle success here immediately
        // App.tsx auth listener will pick up the session on return
    } catch (error: any) {
        console.error(error);
        setErrorMsg(error.message || "Error al conectar con el proveedor.");
            ...
            <>
                {role === 'user' && (
                    ...
                    >
                        ...
                        <span className="text-xs font-black text-gray-700 uppercase tracking-widest">{isSubmitting === 'google' ? 'Conectando...' : 'Entrar con Google'}</span>
                    </button>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">{isSubmitting === 'facebook' ? '...' : 'Facebook'}</span>
                        </button>
                        <button 
                        type="button"
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">{isSubmitting === 'apple' ? '...' : 'Apple'}</span>
                        </button>
                    </div>
